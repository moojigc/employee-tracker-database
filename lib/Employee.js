...
class Employee extends Table {
    ...
    constructor(params) {
        ...
        this.roleTitle = roleTitle ? roleTitle : null;
        this.isManager = isManager ? isManager : false;
        this.departmentID = departmentID ? departmentID : null;
        this.departmentName = departmentName ? departmentName : null;
        this.managerID = managerID ? managerID : null;
    ...
    async addEmployeesPrompt() {
        ...
        if (results.length === 0) {
            ...
            let allManagers = await this.getAllManagers('open', conn);
    ...
    async updateEmployeesPrompt() {
        ...
        switch (action) {
            ...
            case 'Change manager.':
                ...
                const newManager = managers.filter(m => m.id === parseInt(newManagerStr.split(' ')[0]));
                console.log(newManager);
                update = await this.updateRow({ manager_id: newManager.id });
